...
description: Write a client once in Rust and generate node, python, go and rust packages that drive the same compiled binary.
---
...
the [sidecar protocol](/web-re-toolkit/reference/protocol/) is the wire contract between the last two.
...
Return the kind that matches what happened (see the [sidecar protocol](/web-re-toolkit/reference/protocol/)). `target_drift` means the shipped script no longer matches this client. `blocked` means the service answered with a challenge.
