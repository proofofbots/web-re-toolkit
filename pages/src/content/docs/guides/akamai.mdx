...
<Aside type="tip" title="Just want to run it?">
Install the package and copy a working run. This guide is how the client works, not how to use it.
</Aside>

<CardGrid>
  <LinkCard title="Node.js" description="npm install @proofofbot/client-akamai" href="/web-re-toolkit/packages/akamai/node/" />
  <LinkCard title="Python" description="pip install wre-client-akamai" href="/web-re-toolkit/packages/akamai/python/" />
  <LinkCard title="Go" description="go get .../packages/go/clients/akamai" href="/web-re-toolkit/packages/akamai/go/" />
  <LinkCard title="Rust" description="wre-sdk-akamai by path" href="/web-re-toolkit/packages/akamai/rust/" />
</CardGrid>
...
| `random_machine` | `true` | put every session on a different Mac: chip, cores and memory, picked together |
...
| `typed` | a word off the seed | what the visitor types into the page's first text field |
| `pixel` | `true` | run the pixel client when the page serves one |
| `live_xhr` | `true` | let the sensor's own requests leave the sandbox |
| `wire_pace` | `false` | hold each request until the wall clock reaches the sandbox's |
| `warp` | `16` | how much faster the page's clock runs than the wall while a wait is running |
| `load_posts_ms` | `4000` | ceiling on waiting for the script's own load posts to be answered |
| `wait_ms` | `6000` | ceiling on the wait after the input stream, which ends when the edge validates the jar |
| `rounds` | `2` | payloads the host posts per solve, when `live_xhr` is off |
